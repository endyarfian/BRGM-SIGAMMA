...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<?= base_url('dashboard/src/plugins/src/select2/select2.min.css') ?>">
    <link rel="stylesheet" type="text/css" href="<?= base_url('dashboard/src/plugins/css/light/select2/select2.min.css') ?>">
    <link rel="stylesheet" type="text/css" href="<?= base_url('dashboard/src/plugins/css/dark/select2/select2.min.css') ?>">
    ...
    <!--  BEGIN PAGE LEVEL PLUGINS/CUSTOM SCRIPTS SWEET-ALERT  -->
    <link href=" <?= base_url('dashboard/src/assets/css/light/elements/tooltip.css') ?>" rel="stylesheet" type="text/css" />
    <link hhref=" <?= base_url('dashboard/src/assets/css/dark/elements/tooltip.css') ?>" rel="stylesheet" type="text/css" />
    <!--  END PAGE LEVEL PLUGINS/CUSTOM SCRIPTS SWEET-ALERT  -->
    <style>
        ...
        .form-control-file-wrapper:focus-within {
            border-color: #80bdff;
            outline: 0;
            box-shadow: 0 0 0 0.2rem rgba(0, 123, 255, 0.25);
        }
...
<body class="layout-boxed alt-menu">
        ...
        <header class="header navbar navbar-expand-sm expand-header">
            ...
            <ul class="navbar-item flex-row ms-lg-auto ms-0">
                <li class="nav-item theme-toggle-item">
                    <a href="javascript:void(0);" class="nav-link theme-toggle" title="Mode">
                ...
                <li class="nav-item dropdown notification-dropdown">
                    <a href="<?= base_url('/peta-interaktif'); ?>" class="nav-link dropdown-toggle" title="Peta Interaktif">
                ...
                <?php if (in_groups(['developer', 'web-master', 'super-admin', 'gambut-admin', 'mangrove-admin'])) : ?>
                    <li class="nav-item dropdown notification-dropdown">
                        <a href="javascript:void(0);" class="nav-link dropdown-toggle" id="manajemendatabase" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="Manajemen Data">
    ...
    <script>
        ...
        $("#file_excel").change(function() {
            ...
            var fileSize = (this.files[0].size / 1024).toFixed(2); // Konversi dari byte ke KB
            $("#file_info").text(fileName ? fileName : "Pilih File Excel (.xls, .xlsx)");
            $("#file_size").text(fileSize ? `(${fileSize} KB)` : "");
        });
    </script>
    <script>
        ...
        $("#file_excel2").change(function() {
            var fileName = $(this).val().split("\\").pop();
            var fileSize = (this.files[0].size / 1024).toFixed(2); // Konversi dari byte ke KB
            $("#file_info2").text(fileName ? fileName : "Pilih File Excel (.xls, .xlsx)");
            $("#file_size2").text(fileSize ? `(${fileSize} KB)` : "");
        });
    </script>
    <script>
        ...
        $("#file_pdf").change(function() {
            var fileName = $(this).val().split("\\").pop();
            var fileSize = (this.files[0].size / 1024).toFixed(2); // Konversi dari byte ke KB
            $("#info_pdf").text(fileName ? fileName : "Pilih dokumen PDF (.pdf)");
            $("#size_pdf").text(fileSize ? `(${fileSize} KB)` : "");
        });
    </script>
    <!-- NOTIF  -->
    <script>
    ...
    <script>
        <?php
        if (session()->getFlashdata('info2')) {
        ?>
            swal.fire({
                icon: '<?= session()->getFlashdata('info2'); ?>',
                title: '<?= session()->getFlashdata('judul2'); ?>',
                text: '<?= session()->getFlashdata('pesan2'); ?>',
                type: '<?= session()->getFlashdata('info2'); ?>',
            })
    ...
    <script src="<?= base_url('dashboard/src/plugins/src/select2/select2.min.js') ?>"></script>
    <script src="<?= base_url('dashboard/src/plugins/src/select2/custom-select2.js') ?>"></script>
